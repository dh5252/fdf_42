...
/*   Updated: 2023/08/09 21:10:51 by cahn             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	input_values(t_fdf *fdf, char *line, int h)
{
	...
	{
		fdf->map.values[h][w].value = ft_atoi(split[i]);
		find = ft_strnstr(split[i], ",0x", ft_strlen(split[i]));
		if (!find)
			fdf->map.values[h][w++].rgb = return_color("FFFFFF");
		else
			fdf->map.values[h][w++].rgb = return_color(find + 3);
		++i;
	}
	free_split(split);
}
...
void	map_parsing(t_fdf *fdf, char **argv)
{
	...
	fdf->map.values = (t_map_value **)malloc(sizeof(t_map_value *) * \
					fdf->map.height);
	if (!fdf->map.values)
	...
	{
		fdf->map.values[h] = (t_map_value *)malloc(sizeof(t_map_value) * \
							fdf->map.width);
		if (!fdf->map.values[h])
			print_error_exit("Memory allocation Error\n");
